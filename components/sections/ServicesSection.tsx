...
export function ServicesSection() {
  ...
  useEffect(() => {
    fetch('/api/content', { cache: 'no-store' })
      ...
      .then((data) => setCmsServices(Array.isArray(data?.services) ? data.services.filter((s: ServiceContent) => s.active) : []))
      .catch(() => undefined)
  }, [])
    ...
    <section id="services" className="section section-carbon">
      <div className="section-shell">
        <div ref={headRef} className="reveal section-heading section-heading-split">
          ...
          <p className="body-copy heading-note">Serviciile publicate sunt administrabile din Website Studio. Conținutul poate fi schimbat fără modificarea componentelor frontend.</p>
        </div>
