  ...
  return (
    <section id="portfolio" className="section section-obsidian">
      <div className="section-shell">
        ...
        <div className="portfolio-grid">{items.map((entry, index) => <button key={entry.id} onClick={() => setExpanded(index)} className={`portfolio-card portfolio-card-${(index % 4) + 1}`} data-cursor="VIEW"><Image src={entry.coverUrl || '/images/hero-car.jpg'} alt={entry.title || entry.vehicle} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" /><div className="portfolio-overlay" /><div className="portfolio-meta"><span>{entry.service}</span><strong>{entry.title}</strong><small>{entry.finish || entry.material}</small></div><ArrowUpRight className="portfolio-icon" size={18} aria-hidden="true" /></button>)}</div>
      ...
      {item && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label={item.title} onClick={() => setExpanded(null)}><div className="portfolio-modal" onClick={(e) => e.stopPropagation()}><button className="modal-close" onClick={() => setExpanded(null)} aria-label="Închide"><X size={20} /></button><div className="portfolio-modal-media"><Image src={item.coverUrl || '/images/hero-car.jpg'} alt={item.title} fill sizes="90vw" className="object-cover" /></div><div className="portfolio-modal-copy"><span className="eyebrow">{item.service}</span><h3>{item.title}</h3><div className="portfolio-data-grid"><div><span>Finisaj</span><strong>{item.finish || '—'}</strong></div><div><span>Material</span><strong>{item.material || '—'}</strong></div><div><span>Galerie</span><strong>{item.gallery.length}</strong></div></div>{item.description && <p className="body-copy">{item.description}</p>}<MagneticButton variant="accent" href="#quote" onClick={() => setExpanded(null)}>Solicită un proiect similar</MagneticButton></div></div></div>}
    </section>
  )
}
